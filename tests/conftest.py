"""Shared fixtures for the iris-client test suite.

No network is touched: ``mock_iris`` swaps ``httpx.AsyncClient`` for one wired
to an in-memory ``httpx.MockTransport``, so every request the client builds is
...
import iris_client.client as client_mod
...
@pytest.fixture
def load_fixture():
    """Load a JSON fixture body, e.g. ``load_fixture("orders/order_response.json")``."""
